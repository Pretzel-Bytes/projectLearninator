...
class AddSuffix:
    ...
    def __init__(self, my_logger):
        ...
        self.my_sql = AddSuffixMySql(self.my_logger)

    def __log__(self, str_function, str_message, str_level="debug", str_class="AddSuffix",
                str_file="user_suffix/add_suffix"):
    ...
    def start_add_suffix(self, str_suffix):
        dict_return = dict()
        ...
        dict_check_if_exists = self.my_sql.check_if_suffix_exists(str_suffix)
        ...
        if dict_check_if_exists["bool_exists"]:
            ...
            return dict_return
        dict_add_prefix = self.my_sql.add_suffix(str_suffix)
        if dict_add_prefix['bool_error']:
            dict_return['str_error_text'] = dict_add_prefix['str_error_text']
            dict_return["bool_error"] = True
            return dict_return
        dict_return['bool_suffix_inserted'] = dict_add_prefix['bool_suffix_inserted']
        return dict_return


class AddSuffixMySql:
    my_logger = None
    my_sql = MySQL()
    ...
    def __log__(self, str_function, str_message, str_level="debug", str_class="AddSuffixMySql",
                str_file="user_suffix/add_suffix"):
        # str_level = ['debug', 'error', 'critical']
        if str_level == 'debug':
            self.my_logger.debug("{}.{}.{} | {}".format(str_file, str_class, str_function, str_message))
    ...
    def check_if_suffix_exists(self, str_suffix):
        ...
        try:
            ...
            dict_result_query = self.my_sql.select_data(table_name='core_authentication_user_suffix',
                                                        ...
                                                        conditions=[['user_suffix_text', '=', str_suffix]])
    ...
    def add_suffix(self, str_suffix):
        str_function_name = "add_suffix"
        self.__log__(str_function_name, "Started")
        dict_return = dict()
        dict_return["bool_error"] = False
        dict_return['str_error_text'] = None
        dict_return['bool_suffix_inserted'] = False
        try:
            str_id_prefix = str(uuid4())
            self.__log__(str_function_name, "Initializing MySQL Connection")
            dict_result_query = self.my_sql.insert_data(table_name='core_authentication_user_suffix',
                                                        columns=['id_user_suffix', 'user_suffix_text'],
                                                        values=[str_id_prefix, str_suffix])
            dict_return['bool_error'] = dict_result_query['bool_error']
            dict_return['str_error_text'] = dict_result_query['str_error_text']
            int_row_count = dict_result_query['int_row_count']
            if int_row_count == 1:  # Got One Row as Expected
                self.__log__(str_function_name, "Success 1 Row Added")
                dict_return['bool_suffix_inserted'] = True
            else:
