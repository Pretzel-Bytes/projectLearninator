...
class ChangePrefix:
    ...
    def __init__(self, my_logger):
        ...
        self.__log__("Initialization", "ChangePrefix")
        self.my_sql = ChangePrefixMySql(self.my_logger)
    ...
    def start_change_prefix(self, str_prefix_id, str_prefix_text):
        dict_return = dict()
        ...
        dict_check_if_prefix_exists = self.my_sql.check_if_prefix_exists(str_prefix_text)
        if dict_check_if_prefix_exists['bool_error']:
            dict_return["bool_error"] = True
            dict_return['str_error_text'] = dict_check_if_prefix_exists['str_error_text']
            return dict_return
        if dict_check_if_prefix_exists['bool_exists']:
            dict_return['bool_exists'] = True
            return dict_return
        dict_change_prefix = self.my_sql.change_prefix(str_prefix_id, str_prefix_text)
        if dict_change_prefix['bool_error']:
            dict_return["bool_error"] = True
            dict_return['str_error_text'] = dict_change_prefix['str_error_text']
            return dict_return
        dict_return["bool_changed"] = dict_change_prefix['bool_updated']
        return dict_return


class ChangePrefixMySql:
    my_logger = None
    my_sql = MySQL()
    ...
    def __log__(self, str_function, str_message, str_level="debug", str_class="ChangePrefixMySql",
                str_file="user_prefix/change_prefix"):
        # str_level = ['debug', 'error', 'critical']
        if str_level == 'debug':
            self.my_logger.debug("{}.{}.{} | {}".format(str_file, str_class, str_function, str_message))
    ...
    def check_if_prefix_exists(self, str_prefix_text):
        ...
        try:
            ...
            dict_result_query = self.my_sql.select_data(table_name='core_authentication_user_prefix',
                                                        ...
                                                        conditions=[['user_prefix_text', '=', str_prefix_text]])
    ...
    def change_prefix(self, str_prefix_id, str_prefix_text):
        str_function_name = "change_prefix"
        self.__log__(str_function_name, "Started")
        dict_return = dict()
        dict_return["bool_error"] = False
        dict_return['str_error_text'] = None
        ...
        try:
            ...
            dict_result_query = self.my_sql.update_data(table_name='core_authentication_user_prefix',
                                                        column_value_pair=[['user_prefix_text', str_prefix_text]],
            ...
            if int_row_count == 1:  # Got One Row as Expected
                ...
                dict_return["bool_error"] = True
                dict_return['str_error_text'] = 'Unknown Error Updating Prefix'
                dict_return['bool_updated'] = False
        except Exception as e:
            str_error_id = str(uuid4())
            dict_return["bool_error"] = True
