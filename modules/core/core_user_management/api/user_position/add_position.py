...
class AddPosition:
    ...
    def __init__(self, my_logger):
        ...
        self.my_sql = AddPositionMySql(self.my_logger)

    def __log__(self, str_function, str_message, str_level="debug", str_class="AddPosition",
                str_file="user_position/add_position"):
    ...
    def start_add_position(self, str_position):
        dict_return = dict()
        ...
        dict_check_if_exists = self.my_sql.check_if_position_exists(str_position)
        ...
        if dict_check_if_exists["bool_exists"]:
            ...
            return dict_return
        dict_add_prefix = self.my_sql.add_position(str_position)
        if dict_add_prefix['bool_error']:
            dict_return['str_error_text'] = dict_add_prefix['str_error_text']
            dict_return["bool_error"] = True
            return dict_return
        dict_return['bool_position_inserted'] = dict_add_prefix['bool_position_inserted']
        return dict_return


class AddPositionMySql:
    my_logger = None
    my_sql = MySQL()
    ...
    def __log__(self, str_function, str_message, str_level="debug", str_class="AddPositionMySql",
                str_file="user_position/add_position"):
        # str_level = ['debug', 'error', 'critical']
        if str_level == 'debug':
            self.my_logger.debug("{}.{}.{} | {}".format(str_file, str_class, str_function, str_message))
    ...
    def check_if_position_exists(self, str_position):
        ...
        try:
            ...
            dict_result_query = self.my_sql.select_data(table_name='core_authentication_user_position',
                                                        ...
                                                        conditions=[['user_position_text', '=', str_position]])
    ...
    def add_position(self, str_position):
        str_function_name = "add_position"
        self.__log__(str_function_name, "Started")
        dict_return = dict()
        dict_return["bool_error"] = False
        dict_return['str_error_text'] = None
        dict_return['bool_position_inserted'] = False
        try:
            str_id_prefix = str(uuid4())
            self.__log__(str_function_name, "Initializing MySQL Connection")
            dict_result_query = self.my_sql.insert_data(table_name='core_authentication_user_position',
                                                        columns=['id_user_position', 'user_position_text'],
                                                        values=[str_id_prefix, str_position])
            dict_return['bool_error'] = dict_result_query['bool_error']
            dict_return['str_error_text'] = dict_result_query['str_error_text']
            int_row_count = dict_result_query['int_row_count']
            if int_row_count == 1:  # Got One Row as Expected
                self.__log__(str_function_name, "Success 1 Row Added")
                dict_return['bool_position_inserted'] = True
            else:
