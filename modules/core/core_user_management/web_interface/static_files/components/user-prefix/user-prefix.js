import { UserPrefixTemplate } from "./user-prefix-template.html.js";

const UserPrefix = {
    template: UserPrefixTemplate,
    mounted:function () {
       // Functions and Commands they get run when loaded
       this.get_prefix_listing();
    },
    props: {
        parentData: {
            type: Object,
    ...
    data: function () {
        return {
            ...
            arr_prefix: [],
            bool_loading_user_prefix: false,
            str_id_prefix_selected: '!',
        }
    },
    computed: {
        bool_no_prefix_selected() {
            if (this.str_id_prefix_selected == '!' || this.str_id_prefix_selected == ''){
                console.log("Disable");
                return true;
            }else{
                return false;
    ...
    methods: {
        ...
        get_prefix_listing: async function () {
            // console.log("here:");
            this.arr_prefix = [];
            this.bool_loading_user_prefix = true;
            await sleep(1000)
            // Start authentication attempt
            this.arr_prefix = await get_user_prefix_listing();
            // console.log( this.arr_prefix)
            this.bool_loading_user_prefix = false;
            await sleep(50)
...
async function get_user_prefix_listing() {
    ...
    await axios.get('/api/core_user_management/user_prefix__get_prefix_listing', {
        ...
        .catch(function (error) { // Error result in axios
            sweetAlert.fire({
                title: 'Error Loading User Prefix List',
